...
export class ContentChoicesComponent implements OnInit   {
  ...
  @Input() receivedDcFormatArray0:DcFormatListItem[] 
  @Input() receivedDcContentChoiceArray0:ContentChoice[] 
  @Input() receivedDcSkuFormatArray0:SkuFormat[] 
  @Input() receivedDcFormatDetailArray0:FormatDetail[] 
  ...
  myDcFormatDetailsArray0:FormatDetail[] 
  litOrJwf = '' //jan 2020 literal or jde work field.  used after daco fetch.

  @ViewChild('litInputHtml', {static: false})   litInputName: ElementRef;
  @ViewChild('pgIdSaveHtml', {static: false})   pgIdSave:     ElementRef;
  @ViewChild('pg0Or1SaveHtml', {static: false}) pg0Or1Save:   ElementRef;
  @ViewChild('maskInputHtml', {static: false})  maskInputName: ElementRef;
  ...
  ngOnInit() {
    //console.log('init ContentChoicesComponent')
  ...
  promptLitInput(pgParmIn: ProductGridListItem) {
    //console.log('running promptLitInput'); // Feb2019  
...
// }
  ...
  dfCountFmtUsage() {
    ...
    for (let i = 0; i < this.dateCodeFormatArray0.length; i++) {
      ...
      if (k > -1) {
        this.dateCodeFormatArray0[i].dfUsageCount = this.dfFreqArray0[k].formatUsageCount;
      } else {
        this.dateCodeFormatArray0[i].dfUsageCount = 0
      }
  ...
  dfFilter() {
    ...
    this.dateCodeFormatArray1 =
      ...
      this.dfSort() //Jan2020 new data needs to be sorted.
  ...
  dfFilterFun3d(fdParmIn: FormatDetail, pgParmIn: ProductGridListItem) {
    ...
    if (pgParmIn.pgMask > ' '
      ...
      if (fdParmIn.fdMask.toString().indexOf(pgParmIn.pgMask) == -1) // filter this out
  ...
  dfFilterBum3d(fdParmIn: FormatDetail, pgParmIn: ProductGridListItem) {
    ...
    if (pgParmIn.pgMask > ' '
      ...
      if (fdParmIn.fdMask.toString().indexOf(pgParmIn.pgMask) != -1)// filter this in
      { }
      else { mm = 'n' }
    }  // end of compare for literal
  ...
  dfFilterGum3() {
    ...
    for (i = 0; i < this.dateCodeFormatArray0.length; i++) {
      this.selectedDF = this.dateCodeFormatArray0[i];
      if (this.selectedDF.dfFilterInOut.toString().indexOf('out') === -1) {
    ...
    for (j = 0; j < this.dateCodeFormatArray0.length; j++) {
      ...
      if (this.selectedDF.dfFilterInOut.toString().indexOf('dfSfMatch') === -1) {
  ...
  dfSort(){    
    this.dateCodeFormatArray1.sort(function (a, b) {
      if (a.dfFormatKey.toString().padStart(4,'0') < b.dfFormatKey.toString().padStart(4,'0')) {
        return -1;
      }
      if (a.dfFormatKey.toString().padStart(4,'0') > b.dfFormatKey.toString().padStart(4,'0')) {
        return 1;
  ...
  fdSort(){
    // sort by formatKey + row + rowSlot
    // part1 = formatKey is equal, Row -is equal. use rowslot.
    // part2 = formatKey is equal,  use row.  
    // part3 = part 3-1 and part1 3-2. use formatKey. 
    this.formatDetailArray0.sort(function (a, b) {
      if (a.fdFormatKey.toString().padStart(4,'0') == b.fdFormatKey.toString().padStart(4,'0')) {
        //  format key is equal. compare row.
        if ( a.fdRow.toString() == b.fdRow.toString() ) {
          //   row is equal.  compare row slot.
          if (a.fdRowSlot.toString() < b.fdRowSlot.toString() ) {
            return -1;
          }
          if (a.fdRowSlot.toString() > b.fdRowSlot.toString() ) {
            return +1;
      ...
      return 0;  //get here if there is a tie.
    });
  } //end fdSort
  flFilter() {
  ...
  initDummyArrays() {
    ...
    this.receivedDcFormatArray0 = this.dateCodeFormatArray0 //init the @input array
    ...
    this.receivedDcFormatDetailArray0 = this.formatDetailArray0;
    this.skuFormatArray0 = [
      {
        id: 111,
        sfSku: 'BLA',
    ...
    this.receivedDcSkuFormatArray0 = this.skuFormatArray0  //init the @input array
  ...
  ccCount() {
    ...
    for (let i = 0; i < this.contentChoiceArray1.length; i++) {
      ...
      if (k>-1){
        this.contentChoiceArray1[i].contentUsageCount = this.ccFreqArray0[k].contentUsageCount;
      } else {
        this.contentChoiceArray1[i].contentUsageCount = 0
      }
